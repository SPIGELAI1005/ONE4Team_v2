...
- **AI 4 T (`/co-trainer`)**: Club-scoped chat with structured context and fair-use scope (training, match prep, club ops — not general news/shopping). Per-club LLM keys in **Settings → Club → AI provider** or platform fallback via Edge secrets. **Pro plan**, active trial subscription, or **feature trial** required. Settings shows **connection status** and **Test connection**. **Agent tab** (trainer/admin only): propose → confirm → execute workflows with audit trail; **dark-mode composer** matches dashboard theme. **Follow-up chips**, persona hint for dual-role users, scoped context for player/member/public embed. **Voice** input/output (Web Speech API). Dashboard **AI 4 T this week** usage card (club admin). **Public club modal** (Chat | Agent | Guide with **role-based** prompts and Agent gate; light club composer styling). Server role prompts in **`co-trainer`** Edge (deploy after pull). See **`DEPLOYMENT.md` § AI 4 T**, **`docs/AI4T_RELEASE_REVIEW.md`**, and **§ AI 4 T Agent**.
