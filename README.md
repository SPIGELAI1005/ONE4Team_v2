...
- **Club Page Admin**: Manage public club page (branding, contact, social, SEO, **which sections** appear on `/club/:slug` via `public_page_sections`); publication status badges, responsive preview viewports, hero overlay toggle + strength slider; **neutral** platform default hero assets (no pilot-club photo bleed)
...
- **Communication (`/communication`)**: Club chat channels, announcements with poster upload, **message forward** to WhatsApp/native/copy with ONE4Team attribution; **pagination footer** with accurate counts and EN/DE labels; **dashboard mobile layout (2026-07-07)** — channel dropdown, scrollable thread, **fixed gold-bordered composer**; embedded public modal **mobile layout**; **External Bridge (Beta)** for WhatsApp Business API / Telegram (setup guide: **`docs/backlog/WHATSAPP_EXTERNAL_BRIDGE_SETUP.md`**)
