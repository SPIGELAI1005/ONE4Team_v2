...
- **Communication (`/communication`)**: Club chat channels, announcements with poster upload, **message forward** to WhatsApp/native/copy with ONE4Team attribution; **pagination footer** with accurate counts and EN/DE labels; embedded public modal **mobile layout** (channel dropdown, scrollable thread); **External Bridge (Beta)** for WhatsApp Business API / Telegram (setup guide: **`docs/backlog/WHATSAPP_EXTERNAL_BRIDGE_SETUP.md`**)
...
- **AI 4 T (`/co-trainer`)**: Club-scoped chat with structured context and fair-use scope; per-club LLM keys in **Settings → Club → AI provider**; **Agent tab** (trainer/admin); voice STT/TTS; public club modal with role-scoped Guide/Agent. See **`DEPLOYMENT.md` § AI 4 T**.
