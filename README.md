...
- **Dashboard**: Role-based views (Admin, Trainer, Player, **Member**, Sponsor, **Supplier/partner personas**) with personalized greeting; **persona-gated** messages and tasks (player = team scope + own tasks; member = club-wide announcements/general, events-only upcoming); **dual-world** club vs partner portal routes (`/partner-*`, `/supplier-page` **Partner Page**); persona switch in Settings
