...
interface CostBreakdownPieProps {
  data: CostBreakdownSlice[];
  formatter: (value: number) => string;
}

export function CostBreakdownPie({ data, formatter }: CostBreakdownPieProps) {
  ...
  return (
    <div className="h-72 w-full">
      <ResponsiveContainer>
        <PieChart>
          <Pie data={data} dataKey="value" nameKey="label" innerRadius={60} outerRadius={95} paddingAngle={2}>
            {data.map((slice) => (
