...
export function PublicClubLiveScoresSection() {
  ...
  return (
    <PublicClubSection title={t.clubPage.liveScoresPublicTitle}>
      <PublicClubCard className="flex flex-col gap-5 px-6 py-7">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
          ...
          <div className="min-w-0 flex-1 space-y-2">
            ...
            {liveMatches.length === 0 ? (
              <p className="text-xs text-[color:var(--club-muted)]">{t.clubPage.liveScoresNoLive}</p>
            ) : (
              <ul className="space-y-2 pt-1">
                {liveMatches.slice(0, 3).map((m) => (
                  <li key={m.id}>
                    ...
                    >
                      <span className="font-medium text-[color:var(--club-foreground)] truncate">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <PublicClubButton appearance="primary" className="w-full sm:w-auto" onClick={liveScoresCta}>
            {user ? t.clubPage.liveScoresCtaSignedIn : t.clubPage.liveScoresCtaSignedOut}
          </PublicClubButton>
          {liveMatches.length > 0 ? (
            <Link
              to={matchesHref}
              className="text-center text-xs font-semibold text-[color:var(--club-primary)] hover:underline sm:text-left"
            >
