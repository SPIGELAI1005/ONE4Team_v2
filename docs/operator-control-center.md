...
**Development build cost** defaults are anchored on the **measured** codebase size from `src/generated/app-loc.json` (regenerated by `npm run loc:count` / `prebuild`; **179,075** lines / **884** files as of 2026-08-18; prior **170,246** / **828** on 2026-08-07). The snapshot also records **operator module** size (`operatorScope`: **17,985** lines / **87** files — Control Center UI + `src/lib/operator*`) for internal planning; the Financials cost model uses **total `src/` LOC**. Two estimation methods:

1. **Lines of code** — `linesOfCode × costPerLine` (default = measured LOC × €3)
2. **Effort** — `personDays × dailyRate` (default 400 × €600)

The LOC field **auto-refreshes** from the build snapshot on load unless the operator marks it as customized (manual edit). Use **Use measured LOC** to clear an override. Cost model edits remain **browser-local** (`localStorage`), not synced via Supabase.
