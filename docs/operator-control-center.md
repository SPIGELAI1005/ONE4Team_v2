...
**Development build cost** defaults are anchored on the **measured** codebase size from `src/generated/app-loc.json` (regenerated by `npm run loc:count` / `prebuild`). Two estimation methods:
