...
**Development build cost** defaults are anchored on the **measured** codebase size from `src/generated/app-loc.json` (regenerated by `npm run loc:count` / `prebuild`; **170,246** lines / **828** files as of 2026-08-07). The snapshot also records **operator module** size (`operatorScope`: **17,985** lines / **87** files — Control Center UI + `src/lib/operator*`) for internal planning; the Financials cost model uses **total `src/` LOC**. Two estimation methods:
