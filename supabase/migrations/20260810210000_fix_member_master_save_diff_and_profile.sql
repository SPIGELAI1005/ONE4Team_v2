-- Only count actually changed master fields; sync linked profile name on identity updates.
...
begin
  ...
  loop
    if to_jsonb(v_existing)->>v_key is distinct from to_jsonb(v_result)->>v_key then
