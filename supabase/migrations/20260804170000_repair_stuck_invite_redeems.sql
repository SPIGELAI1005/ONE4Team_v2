-- Repair invites that created auth users but failed redeem when draft status='joined'
-- violated club_member_drafts_status_check (fixed in 20260804160000).
-- Also harden redeem_club_invite: mark invite used before draft bookkeeping, and never
-- let draft status updates roll back membership + used_at.

-- ---------------------------------------------------------------------------
-- 1) One-shot repair for stuck invitees (auth user exists, unused invite, no membership)
-- ---------------------------------------------------------------------------
do $$
declare
  r record;
...
begin
  for r in
    ...
    where ci.used_at is null
      ...
      and not exists (
        ...
        where cm.club_id = ci.club_id
          and cm.user_id = au.id
      )
  loop
    v_team := nullif(trim(coalesce(r.invite_payload ->> 'team', '')), '');
    v_age_group := nullif(trim(coalesce(r.invite_payload ->> 'age_group', '')), '');
    v_position := nullif(trim(coalesce(r.invite_payload ->> 'position', '')), '');

    begin
      insert into public.club_memberships (
        club_id, user_id, role, status, team, age_group, position
      )
      values (
        r.club_id, r.user_id, r.role, 'active', v_team, v_age_group, v_position
      )
    ...
    if v_membership_id is not null then
      ...
      where d.club_id = r.club_id
        and d.invite_id = r.invite_id
      order by d.updated_at desc nulls last
      limit 1;

      if found then
        v_md := coalesce(v_draft.master_data, '{}'::jsonb) - '__draft_guardian_membership_ids';
        insert into public.club_member_master_records (
          ...
          club_registration_date, team_assignment_date, club_exit_date,
          invoice_reference, player_passport_number, internal_club_number,
          emergency_contact_name, emergency_contact_phone,
          allergies, medical_conditions, medications, medical_notes, nationality,
          onboarding_progress, team_integration_status, squad_status, last_evaluation_date
        )
        values (
          v_membership_id, r.club_id,
          nullif(trim(coalesce(v_md ->> 'first_name', '')), ''),
          nullif(trim(coalesce(v_md ->> 'last_name', '')), ''),
          case when (v_md ->> 'sex') in ('male', 'female', 'other', 'prefer_not_to_say') then (v_md ->> 'sex') else null end,
  ...
  raise notice 'repaired_stuck_invite_memberships=%', v_repaired;
end $$;

-- ---------------------------------------------------------------------------
-- 2) Harden redeem: used_at before draft status; draft status never aborts redeem
-- ---------------------------------------------------------------------------
...
begin
  ...
  if not found then
    begin
      ...
      values (
        v_inv.club_id, v_user_id, v_inv.role, 'active', v_team, v_age_group, v_position
      );
    exception
      when unique_violation then
        update public.club_memberships cm
    ...
    loop
      ...
      begin
        insert into public.club_member_guardian_links (
          club_id, guardian_membership_id, ward_membership_id, relationship
        )
        values (
          v_inv.club_id, v_guardian_text::uuid, v_ward_id, 'guardian'
        )
  ...
  if v_ward_id is not null then
    ...
    if found then
      ...
      exception
        when others then
          -- Master-data merge must not block membership / invite redeem.
          null;
      end;
    end if;
  end if;

  -- Mark invite used before draft bookkeeping so a draft constraint cannot orphan accounts.
  update public.club_invites
  set used_at = now()
  where id = v_inv.id;
