-- Platform-wide user directory for the ONE4Team Control Center.
-- Returns only operator-safe fields; visibility varies by platform role.

create or replace function public.mask_operator_email(_email text, _detail_level text)
returns text
language sql
immutable
as $$
  select case
    when _email is null or length(trim(_email)) = 0 then null
    when _detail_level in ('full', 'support') then _email
...
create or replace function public.get_operator_user_detail(_user_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  select jsonb_build_object(
    ...
    'last_active_at', ua.last_active_at,
    'platform_role', ua.platform_role,
    'platform_status', ua.platform_status,
    'status', ua.account_status,
    'invitation_status', ua.invitation_status,
    'detail_level', detail_level,
    'clubs', ua.clubs,
    'recent_activity', coalesce((
      ...
      left join public.clubs c on c.id = ue.club_id
    ), '[]'::jsonb),
    'recent_audit', coalesce((
      ...
      from (
        ...
        where al.actor_user_id = _user_id
        order by al.created_at desc
        limit 12
      ) al
  ...
  from (
    ...
    usage_activity as (
      select max(ue.created_at) as last_usage_at
      from public.usage_events ue
      where ue.user_id = _user_id
    ),
    invite_status as (
    ...
    from user_accounts ua
    left join membership_summary ms on ms.user_id = ua.user_id
    left join usage_activity ua_usage on true
    left join invite_status isv on isv.user_id = ua.user_id
  ) ua;
...
revoke all on function public.mask_operator_email(text, text) from public;
...
revoke all on function public.get_operator_user_detail(uuid) from public;
...
grant execute on function public.get_operator_user_detail(uuid) to authenticated;
