-- Fix redeem_club_invite: RETURNS TABLE columns (club_id, role) shadow SQL references.
-- Error: column reference "club_id" is ambiguous
...
begin
  ...
  insert into public.club_memberships as cm (
    club_id,
    user_id,
    role,
    status,
  ...
  do update set
    role = excluded.role,
    status = 'active',
    team = coalesce(excluded.team, cm.team),
    age_group = coalesce(excluded.age_group, cm.age_group),
    position = coalesce(excluded.position, cm.position);
