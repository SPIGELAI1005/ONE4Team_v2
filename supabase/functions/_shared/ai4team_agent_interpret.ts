/**
 * Natural-language → workflow intent extraction for AI4Team Agent.
...
function resolveActivityId(
  ...
  if (dayFilter && candidates.length > 0) return candidates[0].id;
...
function validateWorkflowParams(
  ...
  if (intent === "add_member_draft" && !isAdmin) {
    return de ? "Nur Admins können Mitgliedsentwürfe anlegen." : "Only admins can add member drafts.";
  }
  ...
  if (intent === "cancel_training") {
    if (!params.activity_id) {
      return de
        ? "Welches Training soll abgesagt werden? Nenne Team und Datum."
        : "Which training should be cancelled? Include team and date.";
    }
  }

  if (intent === "notify_trainers") {
    if (!params.title || !params.content) {
      return de ? "Titel und Text der Ankündigung fehlen." : "Announcement title and message are required.";
    }
  }

  if (intent === "add_member_draft") {
    if (!params.email || typeof params.email !== "string" || !params.email.includes("@")) {
      return de ? "E-Mail-Adresse fehlt oder ist ungültig." : "A valid email address is required.";
    }
  }
...
→ {"kind":"workflow","confidence":0.9,"intent":"cancel_training","params":{"team_id":"<aus teams>","date_hint":"tomorrow","activity_hint":"U17"}}
...
→ {"kind":"workflow","confidence":0.9,"intent":"cancel_training","params":{"team_name":"U17","date_hint":"tomorrow","activity_hint":"U17"}}
...
function buildInterpretSystemPrompt(
  ...
  return [
    de
      ? "Du bist der AI4Team Workflow-Interpreter. Extrahiere aus Trainer-Nachrichten (auch gesprochen/kurz) konkrete Vereinsaktionen."
      : "You are the AI4Team workflow interpreter. Extract concrete club actions from trainer messages (including spoken/short commands).",
    "",
    de ? "REGELN:" : "RULES:",
    de
...
export async function interpretAgentMessage(
  ...
  const validationError = validateWorkflowParams(intent, params, language, Boolean(options.isAdmin));
  if (validationError) {
