...
Last updated: 2026-06-14 (admin dashboard live data, financial P&L, German import, role-aware layout, Lucide icons, doc sync)
...
- **Admin dashboard live ops (2026-06-14):** **`club-dashboard-snapshot.ts`** feeds admin KPIs, upcoming schedule, club setup card (name, category, address, public page status, teams/members, links to **`/club/:slug`** and **`/club-page-admin`**). **`dashboard-section-visibility.ts`** — admin dashboard emphasizes finances/ops; trainer/player keep sports widgets (analytics, season progression, achievements); sponsor minimal.
