...
- **Teams member picker (2026-07-16):** Assign any club member from Edit Team (full roster search + emails).
- **AI 4 T branding (2026-07-16):** Red **4** inline label / title icon across dashboard AI surfaces.
- **Allach membership fees (2026-07-16):** Package annual fees + €30 registration shown on join form (provisional amounts).
- **Plan gate UX + guided setup team fix (2026-07-08):** **`PlanGate`** upgrade screen shows **ONE4Team logo + star badge** (not lock) and a **Contact ONE4Team** button under **View Plans** (`mailto:support@one4team.com` with prefilled feature/plan context). **`getFeatureDisplayName()`** in **`plan-limits.ts`**. **Guided setup** team step: removed phantom **`is_active`** from **`teams`** insert (same schema mismatch as club-create RPC; client-only fix). See **`CHANGELOG.md`** § **2026-07-08 (Plan gate UX…)**.
- **Operator Control Center — financials + charts (2026-07-08):** Internal **`/operator`** surface for platform staff (`platform_users`). **Financials** page models MRR/ARR, editable **cost model** (itemized tools, usage drivers, save + comment history in browser), and **development build cost** (LOC × €/line or man-days × daily rate; defaults anchored on ~84k LOC in audit). **Recharts** on Overview (club growth, status pie), Analytics (module usage, active users), Financials (investment timeline with purple **development** cumulative line, cost pie, revenue-by-plan stacked bar). Investment timeline: **series visibility toggles** (hide development → net = revenue − operating only) plus click-to-highlight legends. Chart legends use foreground text; tooltips use popover tokens. **Mobile:** operator cards **`min-w-0`** prevent chart/table overflow on phone; bottom nav scrolls horizontally. **Legal** page: editable body + PDF preview; two-column signatures. **Shell:** scroll-to-top on operator route change; tooltips portaled (info bubbles not clipped). Full EN/DE operator i18n. Docs: **`docs/operator-control-center.md`**, **`docs/operator-control-center-qa.md`**. See **`CHANGELOG.md`** § **2026-07-08**.
- **Dashboard mobile polish + AI 4 T / Messages UX (2026-07-07):** iOS-like mobile dashboard — **`DashboardLayout`** fixed scroll shell + **`MobileBottomNav`** clearance (`--dashboard-bottom-clearance`, `.dashboard-scroll-area`); **`dashboard-page-shell.ts`** typography/toolbar/header tokens; **`DashboardIosSegmentTabs`**, **`DashboardToolbarActions`**; header utility icons **44px** on phone; **`NotificationBell`** portaled + mobile-safe positioning. **AI 4 T (`/co-trainer`):** compact mobile empty state, full-height **`data-dashboard-chat-shell`**, fixed bottom **`Ai4tChatComposer`**, red-highlighted dashboard composer, larger tab/header icons. **Messages (`/communication`):** **`data-dashboard-messages-shell`**, channel sidebar hidden on mobile + dropdown, fixed gold-bordered composer (not red). **Teams Asset Map:** responsive tabs/cards/grids. **Edge:** **`chat-bridge`** CORS allows **`x-correlation-id`** (redeploy). Migrations **`20260707190000`** (club create 409), **`20260707200000`** (marketplace RLS). See **`CHANGELOG.md`** § **2026-07-07 (Dashboard mobile polish…)**.
- **Legal audit + marketing polish (2026-07-07):** Privacy/Terms/cookie copy expanded for AI 4 T, Stripe, sub-processors (**§5**), EU AI Act transparency; Privacy **13** sections; operator todos **LEGAL-OPS-001** (street address), **LEGAL-OPS-002** (HRB). Legal pages use **`.legal-panel`** + opacity-only scroll-in (no top-edge flicker). **41-day** free trial; Pricing **AI 4 T** add-on card (EUR 4/mo + video); DE trial suffix **Tage**; About EN hero 4 lines; **`support@one4team.com`**; Auth logo → home; **`ScrollToTop`**; Matches mobile datetime alignment. See **`CHANGELOG.md`** § **2026-07-07 (Legal audit…)**.
...
- **Stripe:** Sync live Stripe prices to **`plan-catalog.ts`** amounts (**`TASKS.md` PKG-003**) before relying on Pricing checkout for new packages.
