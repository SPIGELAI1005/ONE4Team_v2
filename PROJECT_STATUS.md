...
- **Guardian dual-role + family Members (2026-08-13):** Player-parents and trainer-parents use dashboard **persona switch** (Parent vs Player/Trainer). Family **`/members`** roster = self + linked children. DB **`20260812320000`** auto-assigns **`parent`** on guardian link. Duplicate review dismiss **`20260812310000`**. Operator LOC **178,986** / **883** files. See **`CHANGELOG.md`** § **2026-08-13**.
- **Team Operations expansion (2026-08-10 audit):** Desktop prompt set (Prompts 0–27) mapped to repo — **Waves 1–7 + Phases 18–26 implemented**. Master status: **`docs/TEAM_OPS_EXPANSION_STATUS.md`**. Remaining: E2E scenarios 2–6, i18n sweep, guest invite RPC, UX polish, deferred PWA/waitlist/lineup bridge.
- **Guardian / parent Members + Settings (2026-08-09):** Parents (`parent_supporter`) and any account with **`club_member_guardian_links`** or **`parent`** assignment can open **Members** (family-filtered roster), edit linked children's registry data, and see **Family members you manage** on **Settings → Profile** and **`/my-data`**. RBAC family scope mirrors payments/My Dues. Migration **`20260812290000`** fixes Team Management **Roles tab** updates. See **`CHANGELOG.md`** § **2026-08-09**, **§ 2026-08-13**.
...
- **Operator LOC (2026-08-18):** **179,101** measured lines / **884** files total; **17,985** / **87** operator module (`operatorScope` in **`app-loc.json`**). Prior **2026-08-07:** **170,246** / **828**.
...
- **ONE4AI / LLM (2026-03-28, rebranded AI 4 T 2026-06-14):** Per-club AI configuration (`club_llm_settings`), reliable chat in `CoTrainer.tsx`, Settings **AI provider** card with connection status + health check. Feature trials (`club_feature_trials`) and fair-use scope on Edge. Apply migrations through **`20260614140000`**; deploy LLM Edge functions. See `CHANGELOG.md` § 2026-03-28 and § 2026-06-14 (AI 4 T).
...
- **AI 4 T pilot UX P4-002 (2026-07-01):** Persona-scoped context, Agent tab gated for player/member, follow-up chips, error mapping, history filters, admin usage widget, partner marketplace context, release review doc. Pilot metrics **AI4T-PILOT-001**–**005** closed *(operator OK 2026-07-16)*. See **`CHANGELOG.md`** § **2026-07-01 (AI 4 T pilot UX…)** and **`docs/AI4T_RELEASE_REVIEW.md`**.
